...
const GECERLI_TIPLER = new Set([
  'gunlukBrifing', 'baglamSeridi', 'niyetIzgarasi',
...
function sistemPromptu(b: OgrenciBaglam, env: Env): string {
  return [
    ...
    '- Somut ol: "çok çalış" deme; hangi konu, kaç soru, ne kadar süre gibi uygulanabilir öneriler ver.',
    ...
    '  esas al; günleri ona göre hesapla, tahmin etme. "bugün için"→sadece bugün; "yarın için"→sadece (Bugün+1).',
    '  GEÇMİŞ bir güne ASLA plan koyma; planın ilk günü en erken bugün olabilir.',
    '',
    KART_REHBERI,
    '',
    'Öğrenci bağlamı:',
...
const KART_REHBERI = [
  ...
  '- Konu sinyali ("hafiza"): öğrenci bir KONUDA zorlandığını ya da artık anladığını belli ederse ekle:',
  '  "hafiza": { "konu": "Organik Kimya", "ders": "TYT Kimya", "sinyal": "zorlaniyor"|"anladi" }.',
  '  Zorlanma çok farklı sözlerle gelir, HEPSİ sayılır: "X\'i anlamıyorum/anlamadım", "X\'te zorlanıyorum",',
  '  "X beni zorluyor", "X\'i yapamıyorum", "X\'te takılıyorum", "X\'i kavrayamıyorum/oturtamıyorum",',
  '  "X çok zor geliyor", "X\'te sıkıntı/sorun yaşıyorum", "X\'e zayıfım", "X\'te kötüyüm", "X kabusum".',
  '  Kalıp birebir tutmasa da ANLAM zorlanmaysa EKLE. Konu adı son mesajda net değilse son birkaç mesajdan /',
  '  çözülen sorudan çıkar (az önce limit anlattıysan "bunu anlamadım" derse konu "Limit").',
  '  "zorlaniyor"da şüphedeysen YİNE EKLE (öğrenci sonradan profilden düzeltebilir); yalnız "anladi"da emin',
  '  değilsen KOYMA. EKLEME: genel duygu/motivasyon (yorgunum, canım istemiyor, motivasyonum yok) konu sinyali',
  '  DEĞİL; bir çözümün TEK adımını anlamamak da konu zorlanması değildir (o adımı açıkla, "hafiza" koyma).',
  '',
  ...
  '- Plan: "haftalikPlan" (haftalık çalışma programı). · Formül/özet: "formulKarti". · Moral: "momentum" + "molaRecetesi".',
  ...
  '- molaRecetesi: { baslik, altBaslik?, ogeler:[{icon,metin}] }',
  ...
  '    · gun: TAM Türkçe gün adı ("Pazartesi".."Pazar"). Plan öğrencinin İSTEDİĞİ ilk günden başlar: "bugün"→sadece',
  '      bugün (1 gün), "yarın"→sadece yarın (1 gün), aksi belirtilmedikçe bugünden. Oradan ARDIŞIK ilerle (N gün →',
  '      N ardışık gün; 7\'yi aşınca gün adları tekrar eder). GEÇMİŞ güne ASLA koyma. bugun:true yalnız gün gerçekten bugünse.',
  '    · isler: 1-3 KISA alt görev (takvime adım olur). SORU çözme işinde sayıyı MUTLAKA yaz ("20 soru çöz"); sayısız',
  '      "soru çöz" YAZMA. İş metnine dk yazma (süre güne ait). Soru sayısı GERÇEKÇİ olsun, şişirme yok.',
  '    · sure: günün TOPLAM süresi BİRİMLİ ("90 dk"/"2 saat", çıplak sayı yok) ve işlerle TUTARLI. Tempo (çöz+kontrol):',
  '      TYT ~1 dk/soru; AYT Mat/Fiz ~2.5 dk/soru; Kimya/Biyo/Türkçe ~1.5-2 dk/soru; konu tekrarı 25-45 dk; mola 10 dk.',
  '      Örn: odak "AYT Matematik", isler ["Türev: 12 soru çöz","Limit konu tekrarı"], sure "60 dk". (15 soruya 75 dk = YANLIŞ.)',
  '    · odak: ana ders/konu (kısa). cta.etiket "Takvime ekle". baslik ≤30 karakter, ozet tek kısa cümle.',
  '    · tarihAraligi KISA: "10–19 Haziran" gibi (ay/yıl tekrarı yok).',
  ...
  '    · KAPSAMI öğrencinin DEDİĞİ aralığa göre seç, KENDİLİĞİNDEN DARALTMA: "bugün/bugünü/bugünküleri"→"bugun";',
  '      "bu hafta/haftalık/bu haftaki/bu haftanın programı"→"hafta"; "hepsi/tümü/her şey/komple/tüm programları/',
  '      tüm planları/hepsini sil/takvimi sıfırla/baştan"→"tumu". Öğrenci "bu hafta" dediyse "bugun" SEÇME; tam istediği kapsamı ver.',
